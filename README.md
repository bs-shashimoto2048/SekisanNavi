...
<!--
  Issue #11: 最新mainのUI(Issue #9反映後)を実ブラウザで起動して撮影した
  スクリーンショット。ProjectHeader・図面一覧(DrawingNavigator)・中央Viewer・
  積算コードMaster・盤情報・積算集約・積算明細が1画面で確認できるものを用意した。

  この画面(A1GV2421)には、Backend db/seed.py に既存投入されているダミーの案件情報
  (盤名称: 「高圧受電盤（中部電力ミライズ様）」)がProjectHeaderの表示上に含まれる。
  この文字列自体は本リポジトリの `backend/app/db/seed.py` に既にコミット済みだが、
  README(最初に表示されるページ)へスクリーンショットとして掲載することで露出が
  大きく増すため、**このコミットには画像ファイルをまだ含めていない**。
  取得済みの画像はローカルにのみ保存してあり、掲載可否をIssue #11へ報告して
  ユーザーの判断を仰いでいる。承認後に `docs/assets/screenshot-main.png` として
  追加し、この行を `![Sekisan Navi 画面全体](docs/assets/screenshot-main.png)`
  へ置き換える。
-->

**スクリーンショットは掲載可否の確認待ちのため、このコミットには含まれていません。**
詳細は本READMEの更新PRの説明、および対応するGitHub Issueのコメントを参照してください。
