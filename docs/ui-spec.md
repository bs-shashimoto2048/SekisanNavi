...
- **Phase 1.9: ラベル表示の簡素化**。長いBAN_MEISYOU/BAN_TYPEの説明文はサムネイルへ
  ...
  (Phase 1.8からの方針を継続)。
- **サムネイルラベルの説明文 (Phase 1.11 指示書20章、2026-09 追加修正で共通化・
  簡素化)**。サムネイルラベル(`P{page_no}` / `{面番号}/{盤番号}`)の意味が分かる
  短い説明文`P：ページ、面番号 / 盤番号`を表示する。旧仕様は「外形図」等の
  drawing_typeグループごとに、そのグループがBAN情報(product_df由来のpanels)を
  持つかどうかで文言を出し分けていた(長短2パターン+「その他」は非表示)が、
  今回そのグループ別の出し分けを廃止し、「図面一覧」見出し直下でセクション全体
  1回だけの固定文言表示へ統合した。「クロスリファレンス番号」という表現は
  使わない。長い説明文にはしない。
