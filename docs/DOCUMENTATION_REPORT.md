...
撮影した。実在の企業名・実在の製番・実図面はいずれも画面内に含まれない。
...
**通常運用データ(実案件のManual BBox等)・`decision_events`・
`estimate_confirmations`・`data/master/estimate_master_a.xlsx`はいずれも
変更していない。** 復元後に実データを再表示し、案件情報および積算集約結果が
撮影前と完全に一致することを確認した。
...
復元後、実データを再表示し、案件情報および積算集約結果が撮影前と完全に
一致することを確認した。通常運用データ・アプリのソースコード(`db/seed.py`
含む)はいずれも変更していない。
