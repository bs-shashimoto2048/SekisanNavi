...
- スクリーンショット: 現行デモ(製番`A1GV2421`)の全体画面には、
  `backend/app/db/seed.py`のダミー案件情報(`project_info.panel_name`)に
  実在企業名を含む文字列が含まれ、画面内の複数箇所(ProjectHeaderに加え、
  積算集約の積算確定ボタンラベル等)に製番`A1GV2421`自体も表示される。
  ユーザーからの明示的な方針(実在企業名・実製番を含む画面を掲載しない、
  画像編集による隠蔽・モザイクもしない、架空画像の生成もしない、
  README用画像のためだけにアプリ本体のロジックや通常運用データを変更しない)に
  従って検討した結果、**既存のseed/データ構造だけでは安全なデモ画面を
  用意できないと判断し、今回はスクリーンショットを掲載しなかった**
  (掲載するには`project_info`のダミー値を安全な名称へ変更する等、
  永続的なseedデータの変更が必要になり、これは事前にIssue #11での
  合意が必要な変更のため、今回は見送った)。詳細判断はIssue #11への
  報告コメントを参照。README側にも「未掲載」であることを目立つ形では
  残さず、通常のトップページとして成立する構成にした。
