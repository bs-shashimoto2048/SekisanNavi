...
「この部品・工事にはこのコードを使う」という一覧表(カタログ)です。
上部の「カテゴリ」欄(プルダウンの選択リスト)で種類(箱・単独、内部パネル、
など)を切り替えると、その種類の部品・工事だけが下の表に表示されます。
この一覧から行を選ぶことで、図面上に枠を置く準備ができます(次の章で
説明します)。表にはコード・型式・定格の3項目だけを表示します(価格や
作業時間などの内訳は、この一覧では表示していません)。

以前は画面下段に常に表示され、上部にコード・型式で絞り込む検索欄も
ありましたが、図面をできるだけ大きく見られるよう、現在は次項「⑤ 盤情報」
以降と同じく**図面の上に重ねて表示**され、検索欄は廃止しています。
...
次項⑤〜⑦と同じく、次の操作ができます。
- **表示/非表示**: ツールバー右端の「部品台帳」ボタンで切り替えます。
- **好きな位置へ動かす**: 見出し(「部品台帳」の文字部分)を掴んで
  ドラッグします。
