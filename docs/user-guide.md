...
ボタンの並び)右端の**「盤情報を隠す」/「盤情報を表示」**ボタンで個別に
ON/OFFできます(次項「積算集約」「積算明細」も同じ場所に並んでいます)。

見出し(「▼ 盤情報 ○件」のように書かれた部分)をクリックすると、この領域の
中身だけを折りたたんで隠したり、また元に戻したりできます(表示/非表示の
ボタンとは別の操作です。下の「積算集約」「積算明細」も同じ操作ができます)。
...
されます。ツールバー(元に戻す/やり直すボタンの並び)右端の
**「積算集約を隠す」**ボタンで一時的に非表示にでき、非表示中は同じボタンが
**「積算集約を表示」**に変わります(前項「盤情報」・次項「積算明細」も
同様に個別でON/OFFできます)。
...
Viewerの右下寄りに重ねて表示され、ツールバー右端の**「積算明細を隠す」/
「積算明細を表示」**ボタンで個別にON/OFFできます。
...
画面左上のツールバーにある「積算集約を表示」ボタンを押してください)。
