...
- いちばん右に3つのボタンがあります。
  ...
  - **「積算資料」**: 積算作業の参考資料(PDF)を画面から離れずに参照する
    ためのボタンです。詳しくは
    [「4-12. 積算資料(参考資料PDF)を見る」](#4-12-積算資料参考資料pdfを見る)
    を参照してください。
...
以前は画面下段に常に表示されていましたが、図面をできるだけ大きく見られる
よう、現在は次項「⑤ 盤情報」以降と同じく**図面の上に重ねて表示**されます。
見た目の色が濃い青灰色になっているのは、これが「表示を見るための場所」
ではなく「クリックして次の作業(枠を置く準備)につながる道具」であることを
分かりやすくするためです(表示ON/OFFのボタンも同じ色で、他の3つのボタンとは
少し離れた場所にまとめてあります)。

次項⑤〜⑦と同じく、次の操作ができます。
- **表示/非表示**: ツールバー右端の「積算コードMaster」ボタンで切り替えます。
- **好きな位置へ動かす**: 見出し(「積算コードMaster」の文字部分)を掴んで
  ドラッグします。
- **大きさを変える**: 枠の右下すみをドラッグします。

### ⑤ 盤情報(図面の上にフローティング表示)

現在の案件に含まれる「盤」(制御盤・配電盤などの設備1台1台)の一覧です。
それぞれの盤の寸法(高さ・幅・奥行き)や型式が表示されます。以前は画面右側に
常時表示されていましたが、図面をできるだけ大きく見られるよう、現在は
**図面Viewerの左上寄りに重ねて表示**されます。ツールバー(元に戻す/やり直す
ボタンの並び)右端に「盤情報」というボタンがあり、これを押すたびに
ON/OFFが切り替わります(次項「積算集約」「積算明細」のボタンも同じ場所に
並んでいます)。ボタンの色が濃く塗りつぶされているときが「表示中」、
薄い色のときが「非表示中」です(ボタンの文字自体は常に「盤情報」のまま
変わりません。マウスを乗せると今の状態が「隠す」か「表示する」かの
説明が出ます)。

この「盤情報」・次項の「積算集約」「積算明細」は、いずれも次の3つの操作が
共通してできます。

- **表示/非表示**: 上で説明したツールバー右端のボタンで切り替えます。
...
表示される場所です。以前は画面右側に常時表示されていましたが、図面を
できるだけ大きく見られるよう、現在は**図面Viewerの右上寄りに重ねて表示**
されます。ツールバー(元に戻す/やり直すボタンの並び)右端の「積算集約」
ボタンを押すたびにON/OFFが切り替わります(前項「盤情報」・次項「積算明細」
も同様に個別でON/OFFでき、ボタンの色の濃さで今どちらの状態かが分かります)。
...
置いたか」を1件ずつ振り返るための場所です。「積算集約」と同じく、図面
Viewerの右下寄りに重ねて表示され、ツールバー右端の「積算明細」ボタンで
個別にON/OFFできます。

行をクリックすると、対応する図面のページへ自動的に移動し、該当の枠が
ハイライトされます。

**図面をもっと大きく見たいときは**: 積算集約・積算明細の両方を隠すと、
図面Viewerに何も重ならない状態になり、図面全体をより広く確認できます。
BBoxの位置を確認・編集しながら金額や明細も見たいときは、必要な方だけを
表示に戻してください。
...
図面の上に重ねて表示されている「積算コードMaster」で、これから記録したい
...
図面の右上あたりに重ねて表示される「積算集約」を見ると、今まで置いた枠の
内容が自動的に集計され、金額が表示されています(表示されていない場合は、
画面左上のツールバーにある「積算集約」ボタンを押してください)。

- 「対象」の選択欄で「総合計」を選ぶと案件全体の合計、特定の盤を選ぶと
  その盤だけの小計が表示されます(選べる範囲の詳しい説明は
  [3章の「⑥ 積算集約」](#⑥-積算集約図面の上にフローティング表示)を参照してください)。
...
図面の右下あたりに重ねて表示される「積算明細」では、置いた枠を1件ずつ、
表形式で確認できます(表示されていない場合は、画面左上のツールバーにある
「積算明細」ボタンを押してください)。
...
図面の上に重ねて表示されている「盤情報」の一覧、または図面Viewer上に薄い色で
表示されている盤の範囲をクリックすると、その盤を選択できます。選択すると、
「盤情報」側でも対応するカードが強調表示されます。選択を解除したいときは、
図面の何もない場所をクリックしてください。
...
| 積算資料 | 画面右上の「積算資料」ボタンから参照できる、積算作業の参考資料
  (PDF)。積算コードMasterを置き換えるものではなく、あくまで参考資料です。 |
