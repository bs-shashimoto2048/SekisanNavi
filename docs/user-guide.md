...
されます。図面Viewer右上の**「積算集約を隠す」**ボタンで一時的に非表示にでき、
非表示中は同じ場所のボタンが**「積算集約を表示」**に変わります(次項
「積算明細」も同様に個別でON/OFFできます)。
...
Viewerの右下寄りに重ねて表示され、右上の**「積算明細を隠す」/「積算明細を
表示」**ボタンで個別にON/OFFできます。
...
表形式で確認できます(表示されていない場合は、図面右上の「積算明細を表示」
ボタンを押してください)。
